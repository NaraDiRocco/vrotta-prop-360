...
export async function POST(_request: NextRequest, ctx: { params: Promise<{ project: string; job: string }> }) {
  ...
  if (!lookup.actor || !canManageScenes(lookup.actor)) {
    return NextResponse.json({ error: 'No podés cancelar jobs en este proyecto' }, { status: 403 });
  }

  const { project, job } = await ctx.params;
  try {
    await getRepo().cancelJob(project, job);
    return NextResponse.json({ ok: true });
  } catch (cause) {
    return NextResponse.json(
      { error: cause instanceof Error ? cause.message : 'No se pudo cancelar' },
      { status: 422 },
    );
  }
}
